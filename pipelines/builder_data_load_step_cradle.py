...
class CradleDataLoadingStepBuilder(StepBuilderBase):
    ...
    def validate_configuration(self) -> None:
        ...
        # 3) Check that start_date < end_date
        try:
            s = datetime.fromisoformat(self.config.start_date.replace("Z", "+00:00"))
            e = datetime.fromisoformat(self.config.end_date.replace("Z", "+00:00"))
            if s >= e:
                raise ValueError("start_date must be strictly before end_date.")
        except Exception as ex:
            raise ValueError(f"Invalid date comparison: {ex}")

        # 4) Validate output_path is a valid S3 URI (Pydantic already checks in OutputSpecificationConfig)
        # (The config’s validator will catch it if not.)

        # 5) Validate cluster_type (Pydantic does this in CradleDataLoadConfig)
