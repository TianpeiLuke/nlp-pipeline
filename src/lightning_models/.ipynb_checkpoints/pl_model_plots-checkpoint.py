...
def compute_metrics(
    ...
    for metric in metric_choices:
        ...
        try:
            ...
            elif metric in {"accuracy", "f1_score", "auroc", "average_precision", "precision", "recall"}:
...
def roc_metric_plot(
    ...
    from torchmetrics.functional.classification import roc as binary_roc, auroc as binary_auroc
    ...
    if task == "binary":
        ...
        fpr, tpr, _ = binary_roc(y_pred, y_true, task='binary')
        fpr_val, tpr_val, _ = binary_roc(y_val_pred, y_val_true, task='binary')
        auc = binary_auroc(y_pred, y_true, task='binary')
        auc_val = binary_auroc(y_val_pred, y_val_true, task='binary')
        ...
        # --------------------------
        # torchmetrics.multiclass_roc does NOT support average=None
        # So we manually compute ROC per class using binary logic
        fpr = []
        tpr = []
        auc_scores = []
        for i in range(num_classes):
            binary_target = (y_true == i).int()  # One-vs-rest
            fpr_i, tpr_i, _ = binary_roc(y_pred[:, i], binary_target, task="binary")
            auc_i = binary_auroc(y_pred[:, i], binary_target, task="binary")
            fpr.append(fpr_i)
            tpr.append(tpr_i)
            auc_scores.append(auc_i)

        # Plot all class-wise ROC curves
        plt.figure(figsize=(10, 8))
        for i in range(num_classes):
        ...
        plt.savefig(file_path)
        if writer:
            writer.add_figure("ROC/Multiclass-OneVsRest", plt.gcf(), global_step=global_step)
        plt.close()
        ...
        for avg in ["macro", "weighted"]:
            ...
            fpr_avg, tpr_avg, _ = multiclass_roc(
                y_pred, y_true, num_classes=num_classes, average=avg
            )
            auc_avg = multiclass_auroc(
                y_pred, y_true, num_classes=num_classes, average=avg
            )
...
def pr_metric_plot(
    ...
    if task == "binary":
        ...
        # One-vs-rest
        precision, recall, _ = multiclass_precision_recall_curve(y_pred, y_true, num_classes=num_classes, average=None)
        ap_scores = multiclass_average_precision(y_pred, y_true, num_classes=num_classes, average=None)
        ...
        plt.close()
        if writer:
            writer.add_figure("PR/Multiclass-OneVsRest", plt.gcf(), global_step=global_step)

        # Micro & Macro averages
        for avg in ["macro", "weighted"]:
            # note that multiclass_precision_recall_curve accept 'micro'
            precision_avg, recall_avg, _ = multiclass_precision_recall_curve(
                y_pred, y_true, num_classes=num_classes, average=avg if avg == "macro" else "micro"  
            )
            ap_avg = multiclass_average_precision(
                y_pred, y_true, num_classes=num_classes, average=avg
            )
            plt.figure(figsize=(10, 8))
