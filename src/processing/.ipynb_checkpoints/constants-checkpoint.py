...
Label_combination_mapping = np.array([[0., 1., 2.],
                                 [1., 1., 1.],
                                 [2., 1., 2.]])


aws_translate_supported_lang = ['af', 'sq', 'am', 'ar', 'hy', 'az','bn','bs','bg','ca','zh','zh-TW','hr', 'cs', 'da',
                                'fa-AF', 'nl', 'en', 'et', 'fa', 'tl', 'fi', 'fr', 'fr-CA', 'ka',
                                'de', 'el', 'gu', 'ht', 'ha', 'he', 'hi', 'hu', 'is', 'id', 'ga', 'it', 'ja', 'kn', 
                                'kk', 'ko', 'lv', 'lt', 'mk', 'ms', 'ml', 'mt', 'mr', 'mn', 'no', 'ps', 'pl', 'pt',
                                'pt-PT', 'pa', 'ro', 'ru', 'sr', 'si', 'sk', 'sl', 'so', 'es', 'es-MX', 'sw', 'sv',
                                'ta', 'te', 'th', 'tr', 'uk', 'ur', 'uz', 'vi', 'cy']

spacy_supported_lang = ['en', 'fr', 'de', 'nl', 'ja', 'zh', 'ca', 'hr', 'da', 'fi', 'el',
                        'it', 'ko', 'lt', 'mk', 'xx', 'nb', 'pl', 'pt', 
                        'ro', 'ru', 'es', 'sv', 'uk'
                       ]
...
re_emoji_pattern = re.compile(pattern = "["
...
re_http_url = re.compile(r'''(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'\".,<>?\xab\xbb\u201c\u201d\u2018\u2019]))''', re.M | re.I | re.S)
...
re_matchUPS1      = '(1Z ?[0-9A-Z]{3} ?[0-9A-Z]{3} ?[0-9A-Z]{2} ?[0-9A-Z]{4} ?[0-9A-Z]{3} ?[0-9A-Z])'
...
re_match_shiptrackId_all_us = '(' + '|'.join([re_matchUSPS0, re_matchUPS1, re_matchUPS2, re_matchUSPS1, re_matchUSPS2, re_matchUSPS3,  re_matchUSPS4, re_matchFedex1, re_matchFedex2, re_matchFedex3]) + ')'
...
re_address_1 = "{city_pattern},[ ](?:{state_pattern}|(?i){abbrev_state_pattern})[ ]{zip_pattern}".format(
    city_pattern=re_city, state_pattern=re_state, 
    abbrev_state_pattern=re_state_abbreviations, zip_pattern=re_uszipcode)
...
re_day_backslash_month_backslash_year_date = r'''([0-3]?[0-9]/[0-3]?[0-9]/(?:[0-9]{2})?[0-9]{2})'''
...
Noninformative_tokens = {'dear', 'hi', 'hello', "mr", "mrs.", "customer", "pleasure", "sorry", "however", "appreciation",
                         "hear", "inconvenience", "hope", "thank", "thanks", "appreciated", "sincerely", "best", "wishes",
                         "yours", "apology",
                         "consideration", "regards", "truely", "person", "product", 'cardinal'}

# ==========================  End of Letter Keywords =====================
Farewell_tokens_en = ['Best', 
                   'Cordially yours', 
                   'Best regards', 
                   'Best wishes', 
                   'Fond regards',
...
Farewell_tokens_fr = ['Cordialement',
                      'Cordialement à vous',
                      'Chaleureusement',
                      'Bien amicalement',
                      'Amitiés',
...
Farewell_tokens_de = ['Freundliche Grüße',
...
Farewell_tokens_es = ['Atenciosamente',
                      'Muy atentamente',
                      'Muy cordialmente',
                      'Un saludo',
                      'Un saludo cordial',
...
Farewell_tokens_pt = ['Atentamente',
                      'Saudações cordiais',
...
Farewell_tokens_it = ['Cordialmente',
                      'Cordiali saluti',
                      'Cordiali Saluti',
                      'Un Cordiale Saluto',
...
Farewell_tokens_ja = ['をよろしくお願いいたします',
                     ]  

Farewell_tokens_tr = ['Saygılarımızla',]
                   
Farewell_tokens_nl = ['Met vriendelijke groet',
                     ]                   
                   
Farewell_tokens_sv = ['Med vänliga hälsningar']


Farewell_tokens =   Farewell_tokens_en + \
                    Farewell_tokens_fr + \
                    Farewell_tokens_de + \
                    Farewell_tokens_es + \
                    Farewell_tokens_pt + \
...
Greeting_tokens_tr = ['Sayın',
                       'Merhaba',
                  ]

Greeting_tokens_sv = ['Kära',
                   ]


Greeting_tokens =   Greeting_tokens_en + \
                    Greeting_tokens_fr + \
                    Greeting_tokens_de + \
                    Greeting_tokens_nl + \
                    Greeting_tokens_pt + \
...
RE_AMAZON_MRS_HEADER_SW = re.compile(r'''Kära .* och .*(:|,)[\s]+Detta e-postmeddelande skickas till dig av Amazon för att meddela och bekräfta att ett returtillstånd har begärts för följande artiklar.*(?=Anledning till retur:\s*)''', re.M | re.S)
...
RE_AMAZON_MRS_SIGNATURE_SW = re.compile(r'''Begäran mottagen:.* Med vänliga hälsningar,\s+Amazon.*''', re.M | re.S)
RE_AMAZON_MRS_SIGNATURE_JA = re.compile(r'''返品リクエストの送信日:.*今後ともAmazon.co.jpをよろしくお願いいたします。.*''', re.M | re.S)
RE_AMAZON_MRS_SIGNATURE_PL = re.compile(r'''Żądanie otrzymano:.* Pozdrawiamy,\s+Amazon.*''', re.M | re.S)
