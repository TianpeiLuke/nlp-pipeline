...
def build_collate_batch(input_ids_key: str = 'input_ids', attention_mask_key: str = 'attention_mask'):
    def collate_batch(batch):
        ...
        for key in batch[0]:
            ...
            if all(isinstance(item[key], list) 
                ...
                all_attention_masks = []
                for item in batch:
                    input_chunks = [torch.tensor(chunk[input_ids_key], dtype=torch.long) for chunk in item[key]]
                    mask_chunks = [torch.tensor(chunk[attention_mask_key], dtype=torch.long) for chunk in item[key]]
