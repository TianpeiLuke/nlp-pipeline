...
class XGBoostModelHyperparameters(ModelHyperparameters):
    ...
    num_round: int = Field(
        description="The number of boosting rounds for XGBoost.")
    ...
    model_class: str = Field(
        default="xgboost", 
        description="Model class identifier, set to XGBoost.")
    ...
    def validate_xgboost_hyperparameters(self) -> 'XGBoostModelHyperparameters':
        ...
        if self.tree_method == "gpu_hist" and self.device == -1:
             print(f"Warning: tree_method is '{self.tree_method}' but device is CPU (-1). "
