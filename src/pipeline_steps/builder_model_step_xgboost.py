...
from .config_model_step_xgboost import ModelStepXGBoostConfig
...
class XGBoostModelStepBuilder(StepBuilderBase):
    ...
    def __init__(
        ...
        Args:
            config: A ModelStepXGBoostConfig instance containing all necessary settings.
            sagemaker_session: The SageMaker session object to manage interactions with AWS.
            role: The IAM role ARN to be used by the SageMaker Model.
            notebook_root: The root directory of the notebook environment, used for resolving
                         local paths if necessary.
        """
        if not isinstance(config, ModelStepXGBoostConfig):
            raise ValueError(
                "XGBoostModelStepBuilder requires a ModelStepXGBoostConfig instance."
            )
        super().__init__(
            config=config,
        ...
        self.config: ModelStepXGBoostConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        logger.info("Validating ModelStepXGBoostConfig...")
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [None, ""]:
                raise ValueError(f"ModelStepXGBoostConfig missing required attribute: {attr}")
        
        # If not using XGBoost framework, image_uri is required
        if not self.config.use_xgboost_framework and (
            not hasattr(self.config, "image_uri") or not self.config.image_uri
        ):
            raise ValueError("image_uri must be provided when use_xgboost_framework is False")
        
        logger.info("ModelStepXGBoostConfig validation succeeded.")
    ...
    def _match_custom_properties(self, inputs: Dict[str, Any], input_requirements: Dict[str, str], 
        ...
        """
        Match custom properties specific to XGBoostModel step.
    ...
    def create_step(self, **kwargs) -> ModelStep:
        ...
        # Create the model
        if self.config.use_xgboost_framework:
