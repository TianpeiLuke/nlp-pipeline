...
class ModelHyperparameters(BaseModel):
    ...
    full_field_list: List[str] = Field(
        description="Full list of original field names.")
    ...
    device: int = Field(
        default=-1, 
        description="Device ID for training (-1 for CPU).")
    ...
    metric_choices: List[str] = Field(
        default=['f1_score', 'auroc'], 
        description="Metric choices for evaluation.")
