...
class BSMModelHyperparameters(ModelHyperparameters):
    ...
    text_name: str = Field(
        description="Name of the primary text field to be processed")
    ...
    momentum: float = Field(
        default=0.9, 
        description="Momentum for SGD optimizer (if SGD is chosen)")
    ...
    early_stop_metric: str = Field(
        default='val_loss', 
        description="Metric for early stopping")
    ...
    load_ckpt: bool = Field(
        default=False, 
        description="Load checkpoint flag")
    ...
    chunk_trancate: bool = Field(
        default=True, 
        description="Chunk truncation flag for long texts") # Typo 'trancate' kept as per original
    ...
    text_input_ids_key: str = Field(
        default='input_ids', 
        description="Key name for input_ids from tokenizer output")
    ...
    num_channels: List[int] = Field(
        default=[100, 100], 
        description="Number of channels for convolutional layers")
    ...
    kernel_size: List[int] = Field(
        default=[3, 5, 7], 
        description="Kernel sizes for convolutional layers")
    
    is_embeddings_trainable: bool = Field(
        default=True, 
        description="Trainable embeddings flag")
    
    # For BERT fine-tuning
    pretrained_embedding: bool = Field(
        default=True, 
        description="Use pretrained embeddings")
    
    reinit_layers: int = Field(
        default=2, 
        description="Number of layers to reinitialize from pretrained model")
    
    reinit_pooler: bool = Field(
        default=True, 
        description="Reinitialize pooler layer flag")
    
    # For Multimodal BERT
    hidden_common_dim: int = Field(
        default=100, 
        description="Common hidden dimension for multimodal model")
    ...
    def validate_bsm_hyperparameters(self) -> 'BSMModelHyperparameters':
        ...
        if len(self.num_channels) != self.num_layers:
            raise ValueError(f"Length of num_channels ({len(self.num_channels)}) must match num_layers ({self.num_layers})")
