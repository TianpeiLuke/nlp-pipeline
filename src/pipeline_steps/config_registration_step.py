...
def create_inference_variable_list(
    ...
    for field_name in numeric_fields:
        if not isinstance(field_name, str):
            raise ValueError(f"Field name must be string, got {type(field_name)} for: {field_name}")
    ...
    if output_format not in ['dict', 'list']:
        raise ValueError(f"Output format must be 'dict' or 'list', got: {output_format}")
...
class RegistrationConfig(BasePipelineConfig):
        ...
        if isinstance(v, dict):
            ...
            for key, value in v.items():
                ...
                if isinstance(value, str):
                    ...
                    raise ValueError(f"Value must be string or VariableType, got: {type(value)}")
    ...
    @classmethod
    def validate_output_variable_list(cls, v: Dict[str, Union[VariableType, str]]) -> Dict[str, str]:
