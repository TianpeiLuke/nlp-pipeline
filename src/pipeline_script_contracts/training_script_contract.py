...
class TrainingScriptContract(BaseModel):
    ...
    def validate_input_paths(cls, v: Dict[str, str]) -> Dict[str, str]:
        ...
        for logical_name, path in v.items():
            if not any(path.startswith(prefix) for prefix in valid_prefixes):
                raise ValueError(f'Input path for {logical_name} must start with one of {valid_prefixes}, got: {path}')
        return v
    ...
    def validate_output_paths(cls, v: Dict[str, str]) -> Dict[str, str]:
        ...
        for logical_name, path in v.items():
            if not any(path.startswith(prefix) for prefix in valid_prefixes):
                raise ValueError(f'Output path for {logical_name} must start with one of {valid_prefixes}, got: {path}')
        return v
    ...
    def _validate_against_analysis(self, analyzer: 'TrainingScriptAnalyzer') -> ValidationResult:
        ...
        for path in unexpected_paths:
            if any(path.startswith(prefix) for prefix in ["/opt/ml/input/data", "/opt/ml/input/config"]):
...
class TrainingScriptAnalyzer(ScriptAnalyzer):
    ...
    def get_output_paths(self) -> Set[str]:
        ...
        if self._output_paths is None:
            ...
            training_output_prefixes = ["/opt/ml/model", "/opt/ml/output/data", "/opt/ml/checkpoints"]
