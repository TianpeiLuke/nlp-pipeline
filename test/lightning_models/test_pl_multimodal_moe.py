# test/test_pl_multimodal_moe.py
import unittest
...
from src.lightning_models.pl_multimodal_moe import MultimodalBertMoE
...
class MockTextBertBase(torch.nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # The output dimension is important for the MoE fusion layer
...
class TestMultimodalBertMoE(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {
            ...
            "fusion_dim": 24, # For the MixtureOfExperts module
    ...
    def test_model_initialization(self):
        ...
        with patch('src.lightning_models.pl_multimodal_moe.TextBertBase', MockTextBertBase), \
             patch('src.lightning_models.pl_multimodal_moe.TabAE', MockTabAE):
            model = MultimodalBertMoE(self.config)
            
            self.assertEqual(model.task, "binary")
            self.assertIsInstance(model.moe_fusion, torch.nn.Module)
            self.assertEqual(model.moe_fusion.text_proj.in_features, self.config["text_dim"])
            self.assertEqual(model.moe_fusion.tab_proj.in_features, self.config["tab_dim"])
            self.assertEqual(model.moe_fusion.text_proj.out_features, self.config["fusion_dim"])
            # Check the final classifier's input dimension
            self.assertEqual(model.classifier[1].in_features, self.config["fusion_dim"])
            self.assertEqual(model.classifier[1].out_features, self.config["num_classes"])
    ...
    @patch('src.lightning_models.pl_multimodal_moe.all_gather')
    @patch('src.lightning_models.pl_multimodal_moe.compute_metrics')
