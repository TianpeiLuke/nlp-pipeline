...
class TestSimplifiedConfigFieldCategorization(unittest.TestCase):
    """
    Tests for the simplified configuration field categorization logic with flattened structure.
    ...
    def test_different_values_in_specific(self):
        ...
        try:
            ...
            self.assertEqual(found_descriptions, {"Test Pipeline 1", "Test Pipeline 2"})
            self.assertEqual(found_s3_locs, {"s3://test-bucket/test-pipeline-1", "s3://test-bucket/test-pipeline-2"})
    ...
    def test_load_from_simplified_structure(self):
        ...
        try:
            ...
            self.assertEqual(len(loaded_configs), 2, "Should load 2 configs")
            ...
            self.assertEqual(loaded_config_list[0].pipeline_description, "Test Pipeline 1")
            self.assertEqual(loaded_config_list[0].pipeline_s3_loc, "s3://test-bucket/test-pipeline-1")
            
            self.assertEqual(loaded_config_list[1].pipeline_description, "Test Pipeline 2")
            self.assertEqual(loaded_config_list[1].pipeline_s3_loc, "s3://test-bucket/test-pipeline-2")
