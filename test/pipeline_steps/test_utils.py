...
from src.pipeline_steps.utils import merge_and_save_configs, serialize_config, _serialize, _is_likely_static
from src.pipeline_steps.utils import SPECIAL_FIELDS_TO_KEEP_SPECIFIC
...
class TestConfigFieldCategorization(unittest.TestCase):
    ...
    def test_7_special_field_recovery(self):
        ...
        with patch('src.pipeline_steps.utils.SPECIAL_FIELDS_TO_KEEP_SPECIFIC', {'output_schema'}):
            with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as temp_file:
                output_path = temp_file.name
