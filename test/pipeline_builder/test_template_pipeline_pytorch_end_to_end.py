...
from src.pipeline_builder.template_pipeline_pytorch_end_to_end import (
...
class TestPytorchEndToEndTemplateBuilder(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_configs = {
            ...
            'CradleDataLoading_calibration': self.mock_cradle_test_cfg,
            'TabularPreprocessing_training': self.mock_tp_train_cfg,
            'TabularPreprocessing_calibration': self.mock_tp_test_cfg,
            'PytorchTraining': self.mock_pytorch_train_cfg,
            'PytorchModelCreation': self.mock_pytorch_model_cfg,
        ...
        # Patch load_configs
        self.load_configs_patch = patch('src.pipeline_builder.template_pipeline_pytorch_end_to_end.load_configs')
        ...
        self.template_patch = patch('src.pipeline_builder.template_pipeline_pytorch_end_to_end.PipelineBuilderTemplate')
        ...
        # Patch PipelineDAG
        self.dag_patch = patch('src.pipeline_builder.template_pipeline_pytorch_end_to_end.PipelineDAG')
        ...
        def patched_isinstance(obj, classinfo):
            ...
            if obj is self.mock_tp_test_cfg and classinfo is TabularPreprocessingConfig:
                return True
            if obj is self.mock_pytorch_train_cfg and classinfo is PytorchTrainingConfig:
                return True
            if obj is self.mock_pytorch_model_cfg and classinfo is PytorchModelCreationConfig:
                return True
    ...
    def test_find_config_by_type(self):
        ...
        config = _find_config_by_type(self.mock_configs, PytorchTrainingConfig)
        self.assertEqual(config, self.mock_pytorch_train_cfg)
        
        # Test error when no matching config is found
        with self.assertRaises(ValueError):
            _find_config_by_type({}, PytorchTrainingConfig)
    ...
    def test_builder_map(self):
        ...
        self.assertIn("PytorchTrainingStep", BUILDER_MAP)
        self.assertIn("CreatePytorchModelStep", BUILDER_MAP)
